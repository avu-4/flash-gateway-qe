...
test('Submit transfers with manipulated/invalid form values', async ({page}) => {
    ...
    await page.getByRole('textbox', {name: 'Beneficiary Name'}).fill('Dikela*&#$');
    await page.getByRole('textbox', {name: 'Bank'}).fill('FNB**%#$@');
    await page.getByRole('textbox', {name: 'Account Number'}).fill('1234567890MAHLE');
