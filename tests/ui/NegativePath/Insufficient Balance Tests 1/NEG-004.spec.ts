...
test('Multiple transfers attempt to exceed balance-Transfer 1', async ({page}) => {
   ...
   //Assertion
    ...
    await expect(page.getByText('Transfer created successfully!')).toBeVisible({timeout:});

});
   

test('Multiple transfers attempt to exceed balance-Transfer 2', async ({page}) => {
    test.setTimeout(300000);
   ...
   //Assertion
    await page.getByRole('button', {name: 'lock Confirm & Transfer'}).click();
    await expect(page.getByText('Insufficient balance ')).toBeVisible({timeout:150000});
});
